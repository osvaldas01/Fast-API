...
async def buy_advert_package(
    advert_package_id: int,
    current_user: int = Depends(oauth2.get_current_user),
    db: Session=Depends(get_db)
    ):
    ...
    if user.credits < advert_package.price:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not enough credits to buy this package")
